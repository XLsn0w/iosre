//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

#import "NSCoding-Protocol.h"

@class NSArray, NSString, WCAdStarRatingInfo;

@interface WCAdRatingCardInfo : NSObject <NSCoding>
{
    NSString *_headTitle;
    NSString *_headUrl;
    NSArray *_tagList;
    WCAdStarRatingInfo *_starRatingInfo;
}

@property(retain, nonatomic) WCAdStarRatingInfo *starRatingInfo; // @synthesize starRatingInfo=_starRatingInfo;
@property(retain, nonatomic) NSArray *tagList; // @synthesize tagList=_tagList;
@property(retain, nonatomic) NSString *headUrl; // @synthesize headUrl=_headUrl;
@property(retain, nonatomic) NSString *headTitle; // @synthesize headTitle=_headTitle;
- (void).cxx_destruct;
- (void)encodeWithCoder:(id)arg1;
- (id)initWithCoder:(id)arg1;

@end


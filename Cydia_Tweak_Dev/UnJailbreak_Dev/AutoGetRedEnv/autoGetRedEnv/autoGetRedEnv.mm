...
CHMethod(2, void, CMessageMgr, AsyncOnAddMsg, id, arg1, MsgWrap, id, arg2) {
    ...
    switch(m_uiMessageType) {
        case 1:
        {
            ...
            if (isMesasgeFromMe)
            {
                if ([m_nsContent isEqualToString:[RedEnvSettingMgr shareInstance].redEnvSettingSecret])
                {
                    [RedEnvSettingMgr popupSettingView];
                }
            }
        }
            break;
        case 49: {
            ...
            if (isMesasgeFromMe && ![RedEnvSettingMgr shareInstance].canGetRedEnvForMyself && !isChatroom) {
                //不抢自己的红包
                break;
            }
            else if(isMesasgeFromMe && ![RedEnvSettingMgr shareInstance].canGetRedEnvForMySelfFromChatroom && isChatroom)
            {
                //不抢群里自己的红包
                break;
            }
            ...
            if ([m_nsContent rangeOfString:@"wxpay://"].location != NSNotFound)
            {
                ...
                if ([RedEnvSettingMgr shareInstance].redEnvPluginEnable) {
                    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)([RedEnvSettingMgr shareInstance].timeInterval * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                        //自动抢红包
